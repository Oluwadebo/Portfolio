  ...
  return (
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <p className="font-mono text-[10px] text-[#3B82F6] tracking-[.2em] mb-1">ADMIN / PROJECTS</p>
      ...
      <div className="bg-[#0D1220] border border-white/5">
        ...
        <div className="grid grid-cols-12 gap-4 px-5 py-3 border-b border-white/5">
          {["PROJECT", "TAGS", "STATUS", "ACTIONS"].map((h, i) => (
            <p key={h} className={`font-mono text-[9px] text-[#334155] tracking-[.2em] ${
        ...
        {!loading && projects.length === 0 && (
          <div className="p-12 text-center border border-dashed border-white/10 m-4">
            <p className="font-mono text-[10px] text-[#334155] tracking-widest mb-4">NO PROJECTS YET</p>
            <Link href="/admin/projects/new"
        ...
        {!loading && projects.map((project) => (
          <div key={project._id} className="grid grid-cols-12 gap-4 px-5 py-4 border-b border-white/5 last:border-0 hover:bg-white/[0.02] items-center">
            {/* Title */}
            ...
            <div className="col-span-4 flex flex-wrap gap-1">
              {project.tags.slice(0, 3).map((tag) => (
                <span key={tag} className="font-mono text-[9px] px-2 py-0.5 border border-[#3B82F6]/20 text-[#3B82F6]/60">
                  {tag}
                </span>
              ))}
            </div>
            ...
            <div className="col-span-2">
              {project.featured ? (
                <span className="font-mono text-[9px] px-2 py-1 bg-[#3B82F6]/10 text-[#3B82F6] tracking-widest">FEATURED</span>
              ) : (
                <span className="font-mono text-[9px] text-[#334155] tracking-widest">NORMAL</span>
              )}
            </div>
            ...
            <div className="col-span-2 flex items-center justify-end gap-2">
              ...
              <button
                ...
                className="font-mono text-[9px] px-3 py-1.5 border border-red-400/20 text-red-400/60 hover:text-red-400 hover:border-red-400/40 transition-all tracking-widest disabled:opacity-30">
                {deleting === project._id ? "..." : "DEL"}
              </button>
            </div>
          </div>
        ))}
